...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Admin Header */}
      <div className="bg-slate-900 border border-purple-500/30 p-6 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-1">
          ...
          <p className="text-2xl font-black text-white">{orders.length} Active</p>
          <span className="text-[10px] text-amber-400 font-bold">Live Status Synced</span>
        </div>
        ...
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-1">
          ...
          <p className="text-2xl font-black text-white">100% Success</p>
        ...
        <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-1">
          ...
          <p className="text-2xl font-black text-white">0 Breaches</p>
          <span className="text-[10px] text-purple-400 font-bold">Audit Verified</span>
        </div>
      </div>

      {/* MASTER ORDERS CONSOLE */}
      <div className="bg-slate-900/80 p-6 rounded-3xl border border-slate-800 space-y-4 shadow-2xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-3 gap-3">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-700 text-white text-xs rounded-xl pl-9 pr-3 py-2 outline-none focus:border-purple-500"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/90 text-slate-400 uppercase font-mono text-[10px]">
              <tr>
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-slate-500 font-sans">
                    No active orders. Place an order in Customer Portal to see it appear here live!
                  </td>
                </tr>
              ) : (
                filteredOrders.map((ord) => {
                  ...
                  return (
                    <tr key={ord.id} className="hover:bg-slate-950/40 transition-colors">
                      ...
                      <td className="p-3 font-sans">
                        {modeCfg.icon} {modeCfg.title.split(' ')[0]}
                      </td>
                      ...
                      <td className="p-3">
                        <select
                          ...
                          className="bg-slate-950 text-slate-200 border border-slate-700 rounded-lg px-2 py-1 text-[11px] outline-none cursor-pointer focus:border-purple-500"
                        >
            ...
            <tbody className="divide-y divide-slate-800/60 font-mono">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-slate-950/40">
                  ...
                  <td className="p-3 uppercase text-amber-400">{log.role}</td>
